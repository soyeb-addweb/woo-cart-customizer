...
if (! class_exists('Addweb_Cart_Customizer')) :
	class Addweb_Cart_Customizer
	{
		...
		public function addweb_plugin_notice()
		{
			echo sprintf(
				...
				esc_html(__('Sorry, but Child Plugin requires the Woocommerce plugin to be installed and active.', 'woo-cart-customizer'))
			);
		}
	}
	...
	*/
	...
	function addweb_custom_cart_button_text()
	{
		...
		//Add a check to ensure wc_get_product() returns a valid product object before calling any methods on it.
		//senario : gives fetal error on cart page (solved by this check)
		if (!$_product) {
			return esc_html__('Invalid Product', 'woo-cart-customizer');
		}
